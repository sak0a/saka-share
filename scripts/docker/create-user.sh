...
if ! getent group saka-share-group > /dev/null 2>&1; then
    addgroup -g "$PGID" saka-share-group
fi

# Check if a user with PUID exists; if not, create it
if ! id -u saka-share > /dev/null 2>&1; then
    if ! getent passwd "$PUID" > /dev/null 2>&1; then
        adduser -u "$PUID" -G saka-share-group saka-share > /dev/null 2>&1
    else
